  ...
  template: `
    ...
    >
      <header class="flex items-center justify-between">
        <h2 class="text-lg font-semibold">Morning briefing</h2>
        @if (briefing(); as b) {
          <p-button
            label="Regenerate"
            icon="pi pi-refresh"
      ...
      @if (loading() && !briefing()) {
        <div class="flex items-center gap-2 py-6 justify-center">
          ...
          <span class="text-sm text-muted-color">Generating today's briefing…</span>
        </div>
      } @else if (providerNotConfigured()) {
        <div class="flex flex-col items-start gap-2 py-4">
          <p class="text-sm text-muted-color">
            Configure your AI provider to get an AI-generated morning briefing.
          </p>
          <a
            routerLink="/settings"
        ...
        <div class="flex flex-col gap-2">
          <p class="text-xs text-muted-color">
            Generated {{ b.generatedAtUtc | date: 'medium' }}
            · {{ b.model }}
          </p>
          <article
            class="prose prose-sm max-w-none briefing-body"
...
export class MorningBriefingWidgetComponent implements OnInit {
